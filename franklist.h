...
template <typename T> class FrankList;
...
private:
  ...
  private:
    std::string error_message(const std::type_info &info,
                              char const *const func) const;
  };
...
public:
  ...
  public:
    ...
    const const_multi_reverse_iterator &
    operator=(const base_iterator &rhv);                                // O(1)
...
public:
  ...
  const FrankList<value_type> &
  operator=(const FrankList<value_type> &rhv);                         // O(n)
  const FrankList<value_type> &operator=(FrankList<value_type> &&rhv); // O(n)
  const FrankList<value_type> &
  operator=(std::initializer_list<value_type> init); // O(n)
...
public:
  ...
  typename std::enable_if<
      std::is_base_of<const_iterator, iter>::value ||
  ...
  template <typename iter>
  typename std::enable_if<
      std::is_base_of<const_reverse_iterator, iter>::value ||
  ...
  template <typename iter> iter erase(iter pos);       // O(1)
  template <typename iter> iter erase(iter f, iter l); // O(n)
  ...
  typename std::enable_if<std::is_base_of<base_iterator, iter>::value,
                          void>::type
